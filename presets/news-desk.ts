 ...
 *  (operator, 2026-07-28). Base target 1000 words for a floored story (the
 ...
 *  the contract stays.
 *
 *  The base has been raised twice against measured archive length: 500 → 700
 *  on 2026-08-27 (median ran 533 words, thin against what ad reviewers and the
 *  2026 spam updates reward), then 700 → 1000 on 2026-09-10. The prompt asks
 *  for 70–85% of the cap (see `target`), so a base of 1000 puts a floored
 *  story at 700–850 words ≈ 4,200–5,100 characters — between Guardian news and
 *  AP, and past the ~3,000-character band the archive was landing in. */
export function evidenceWordCap(sourceCount: number, evidenceChars: number, maxCap: number): number {
  const bySources = 1000 + 100 * Math.max(0, sourceCount - 3);
  const byEvidence = Math.min(350, Math.max(0, Math.round((evidenceChars - 7000) / 45)));
  return Math.max(1000, Math.min(maxCap, bySources + byEvidence));
}
  ...
  return {
    async run(): Promise<GeneratedPost> {
      ...
      for (const story of stories) {
          ...
          if (opts.onCovered !== undefined && hitSlug !== "") {
            try {
              await opts.onCovered({
                headline: story.headline,
                slug: hitSlug,
                coveredTitle: judgement.topic.title,
                score: 1,
        ...
        if (judgement.kind === "followup") {
          log?.(
            `news-desk: "${story.headline}" follows "${judgement.topic.title}" — writing as a new column`,
          );
        }

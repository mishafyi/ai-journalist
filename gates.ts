...
export const EDIT_RULES: readonly string[] = [
  ...
  "Vary sentence length by what each sentence does: state conclusions and key points in short declarative sentences; carry explanation, ambiguity and complex reasoning in longer ones; follow a long summary sentence with a short one that lands the point; never run three sentences of the same length in a row.",
...
Make surgical changes, not a rewrite — preserve the reporting and the voice, and keep the piece a full-length feature of at least ${deps.editWordFloor ?? 1200} words (fix by rework, not by deletion — losing whole paragraphs to satisfy a bullet above is a failure). Keep every markdown link and heading intact. Output ONLY the finished markdown article, nothing else.
